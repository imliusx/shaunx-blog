---
title: Spring Boot 自动装配原理：从 SpringBootApplication 到 Bean 加载流程
...
这篇文章就从启动入口开始，梳理 Spring Boot 自动装配背后的核心流程。
...
> 图片占位：这里可以补充一张 Spring Boot 启动流程图，展示从 `main()` 到 `ApplicationContext refresh()` 再到 Web 容器启动的过程。
...
> 图片占位：这里可以补充一张自动装配核心流程图，重点展示 `@EnableAutoConfiguration`、`AutoConfigurationImportSelector`、自动配置类清单、条件注解之间的关系。
...
Spring Boot 会先读取候选自动配置类，再通过一系列 `@Conditional` 条件注解决定是否生效。
...
> 图片占位：这里可以补充一张 Bean 生命周期流程图，展示 BeanDefinition、实例化、属性填充、初始化、销毁等阶段。
...
- [Spring Framework Core Technologies: The IoC Container](https://docs.spring.io/spring-framework/reference/core/beans.html)
...
掌握这些内容，Spring Boot 就不再只是“开箱即用”，而是变成一套你能理解、能排查、能扩展的工程化框架。
